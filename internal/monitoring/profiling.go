...
func TrackMemoryController(level memory.MemoryLevel, gcCount uint32) {
	globalProfile.memoryControllerLevel.Store(int32(level))
	globalProfile.memoryControllerGCCount.Store(gcCount)
}

// trackPublishConfirmProfile rastreia confirmações do RabbitMQ (ACK/NACK) para profiling
func trackPublishConfirmProfile(ack bool) {
	if ack {
...
func PrintProfileReport() {
	...
	memLevel := memory.MemoryLevel(globalProfile.memoryControllerLevel.Load())
